...
void REF::on_network_read(const uint8_t* data, int len)
{
	...
	if ((buf.size() == 5) && (buf.data()[0] == 5)){
		...
		out.append(0x04);
		out.append('\x00');
		out.append(QByteArray::fromStdString(m_modeinfo.callsign).toUpper().constData(), 6);
		out.append(10,'\x00');
	...
	if((buf.size() == 0x3a) && (!memcmp(buf.data()+1, header, 5)) ){
		...
		if( (rptr2.simplified() == h.simplified()) || (rptr1.simplified() == h.simplified()) ){
			...
			m_modeinfo.src = mycall.toStdString();
			m_modeinfo.dst = urcall.toStdString();
			m_modeinfo.gw = rptr1.toStdString();
			m_modeinfo.gw2 = rptr2.toStdString();
	...
	if((buf.size() == 0x1d) && (!memcmp(buf.data()+1, header, 5)) ){ //29
		...
		if((buf.data()[16] == 0) && (buf.data()[26] == 0x55) && (buf.data()[27] == 0x2d) && (buf.data()[28] == 0x16)){
            ...
            //m_sd_debug_cnt = 0;
            //for(int i = 0; i < 63; i++){
            //    fprintf(stderr, "%02x ", m_debug_data[i]);
            //}
           // fprintf(stderr, "\n");
		}
        //m_debug_data[m_sd_debug_cnt++] = buf.data()[26] ^ 0x70;
        //m_debug_data[m_sd_debug_cnt++] = buf.data()[27] ^ 0x4f;
        //m_debug_data[m_sd_debug_cnt++] = buf.data()[28] ^ 0x93;

        char c = buf.data()[26] ^ 0x70;

        if( m_sd_sync && !m_sd_gps_cnt && ((c & 0xf0) == 0x50)){
            m_sd_hdr_cnt = (c & 0x0f);
        }
        else if(m_sd_hdr_cnt){
            c = 0;
            m_sd_hdr_cnt = 0;
        }
        if( m_sd_sync && !m_sd_gps_cnt && ((c & 0xf0) == 0x30)){
            m_sd_gps_cnt = (c & 0x0f);
            c = buf.data()[27] ^ 0x4f;
            if( (c == 0x0a) || (c == 0x0d) ){
                m_gps_data.append('\0');
                m_sd_gps_cnt = 0;
                QTextStream(stderr) << "GPS: " << QString(m_gps_data) << Qt::endl;
                if(m_gps_data[0] == '$') notify_log(("GPS: " + QString(m_gps_data)).toStdString());
                m_gps_data.clear();
            }
            else{
                m_sd_gps_cnt--;
        ...
        else if(m_sd_gps_cnt && (buf.data()[16] != 0)){
            if( (c == 0x0a) || (c == 0x0d) ){
                m_gps_data.append('\0');
                m_sd_gps_cnt = 0;
                QTextStream(stderr) << "GPS: " << QString(m_gps_data) << Qt::endl;
        ...
        if(m_sd_sync && (m_sd_txt_seq == 1) && (buf.data()[16] == 1) && (buf.data()[26] == 0x30)){
		   m_user_data[0] = buf.data()[27] ^ 0x4f;
		   m_user_data[1] = buf.data()[28] ^ 0x93;
           ++m_sd_txt_seq;
		}
        if(m_sd_sync && (m_sd_txt_seq == 2) && (buf.data()[16] == 2)){
		   m_user_data[2] = buf.data()[26] ^ 0x70;
		   m_user_data[3] = buf.data()[27] ^ 0x4f;
		   m_user_data[4] = buf.data()[28] ^ 0x93;
           ++m_sd_txt_seq;
        }
        if(m_sd_sync && (m_sd_txt_seq == 3) && ((buf.data()[16] == 3) || (buf.data()[16] == 5)) && (buf.data()[26] == 0x31)){
		   m_user_data[5] = buf.data()[27] ^ 0x4f;
		   m_user_data[6] = buf.data()[28] ^ 0x93;
           ++m_sd_txt_seq;
		}
        if(m_sd_sync && (m_sd_txt_seq == 4) && ((buf.data()[16] == 4) || (buf.data()[16] == 6))){
		   m_user_data[7] = buf.data()[26] ^ 0x70;
		   m_user_data[8] = buf.data()[27] ^ 0x4f;
		   m_user_data[9] = buf.data()[28] ^ 0x93;
           ++m_sd_txt_seq;
		}
        if(m_sd_sync && (m_sd_txt_seq == 5) && ((buf.data()[16] == 5) || (buf.data()[16] == 9)) && (buf.data()[26] == 0x32)){
		   m_user_data[10] = buf.data()[27] ^ 0x4f;
		   m_user_data[11] = buf.data()[28] ^ 0x93;
           ++m_sd_txt_seq;
		}
        if(m_sd_sync && (m_sd_txt_seq == 6) && ((buf.data()[16] == 6) || (buf.data()[16] == 10))){
		   m_user_data[12] = buf.data()[26] ^ 0x70;
		   m_user_data[13] = buf.data()[27] ^ 0x4f;
		   m_user_data[14] = buf.data()[28] ^ 0x93;
           ++m_sd_txt_seq;
		}
        if(m_sd_sync && (m_sd_txt_seq == 7) && ((buf.data()[16] == 7) || (buf.data()[16] == 13)) && (buf.data()[26] == 0x33)){
		   m_user_data[15] = buf.data()[27] ^ 0x4f;
		   m_user_data[16] = buf.data()[28] ^ 0x93;
           ++m_sd_txt_seq;
		}
        if(m_sd_sync && (m_sd_txt_seq == 8) && ((buf.data()[16] == 8) || (buf.data()[16] == 14))){
		   m_user_data[17] = buf.data()[26] ^ 0x70;
		   m_user_data[18] = buf.data()[27] ^ 0x4f;
		   m_user_data[19] = buf.data()[28] ^ 0x93;
           m_user_data[20] = '\0';
           m_sd_txt_seq = 0;
...
void REF::on_network_connected()
{
	uint8_t out[5] = {0x05, 0x00, 0x18, 0x00, 0x01};
	m_udp->write(out, 5);
    ...
    if(m_debug){
        ...
        QString s = "CONN:";
        for(int i = 0; i < 5; ++i){
            s += " " + QString("%1").arg(out[i], 2, 16, QChar('0'));
        }
        debug << s;
    }
}
...
void REF::format_callsign(std::string &s)
{
    ...
    if (p != std::string::npos && p + 1 < t.size()) {
        std::string first = t.substr(0, p);
        size_t q = t.find_first_not_of(' ', p);
        std::string second = (q != std::string::npos) ? t.substr(q) : "";
        s = first;
...
void REF::send_frame(uint8_t *ambe)
{
	...
	if(m_sendheader){
		...
		txdata[12] = 0x02;
		txdata[13] = 0x01;
		txdata[14] = (m_txstreamid >> 8) & 0xff;
		txdata[15] = m_txstreamid & 0xff;
		txdata[16] = 0x80;
		txdata[17] = 0x00;
		txdata[18] = 0x00;
		txdata[19] = 0x00;
		txdata.replace(20, 8, m_txrptr2.c_str());
		txdata.replace(28, 8, m_txrptr1.c_str());
		txdata.replace(36, 8, m_txurcall.c_str());
		txdata.replace(44, 8, m_txmycall.c_str());
	...
	case 1:
		txdata[26] = 0x40 ^ 0x70;
		txdata[27] = m_txusrtxt[0] ^ 0x4f;
		txdata[28] = m_txusrtxt[1] ^ 0x93;
		break;
	case 2:
		txdata[26] = m_txusrtxt[2] ^ 0x70;
		txdata[27] = m_txusrtxt[3] ^ 0x4f;
		txdata[28] = m_txusrtxt[4] ^ 0x93;
		break;
	case 3:
		txdata[26] = 0x41 ^ 0x70;
		txdata[27] = m_txusrtxt[5] ^ 0x4f;
		txdata[28] = m_txusrtxt[6] ^ 0x93;
		break;
	case 4:
		txdata[26] = m_txusrtxt[7] ^ 0x70;
		txdata[27] = m_txusrtxt[8] ^ 0x4f;
		txdata[28] = m_txusrtxt[9] ^ 0x93;
		break;
	case 5:
		txdata[26] = 0x42 ^ 0x70;
		txdata[27] = m_txusrtxt[10] ^ 0x4f;
		txdata[28] = m_txusrtxt[11] ^ 0x93;
		break;
	case 6:
		txdata[26] = m_txusrtxt[12] ^ 0x70;
		txdata[27] = m_txusrtxt[13] ^ 0x4f;
		txdata[28] = m_txusrtxt[14] ^ 0x93;
		break;
	case 7:
		txdata[26] = 0x43 ^ 0x70;
		txdata[27] = m_txusrtxt[15] ^ 0x4f;
		txdata[28] = m_txusrtxt[16] ^ 0x93;
		break;
	case 8:
		txdata[26] = m_txusrtxt[17] ^ 0x70;
		txdata[27] = m_txusrtxt[18] ^ 0x4f;
		txdata[28] = m_txusrtxt[19] ^ 0x93;
		break;
	default:
		txdata[26] = 0x16;
		txdata[27] = 0x29;
